...
firstBotMessage();
  ...
  graph.addEdge("Out patients ward", "Operation Theater", 10);
...
function getHardResponse(userText) {
    ...
    if(userText>=1 && userText <= 5){
        ...
        if(path[1]=="Emergency ward")
          botResponse = "From "+starting+" "+key[2]+" "+g[0]+" feet to "+path[1];
        else
          botResponse = "From "+ starting+" "+ key[0]+" "+ g[0] +" feet to " +path[1]; 
        for(let i=1;i<path.length-1;i++){
          if(path[i+1]=="Emergency ward" || path[i+1]== "Out patients ward" )
            botResponse = botResponse + " "+" then " +key[3]+" "+ g[i] +" feet to " +path[i+1];
          if(path[i+1]=="ICU" || path[i+1]=="Operation theatre")
          botResponse = botResponse + " "+" then " +key[0]+" "+ g[i] +" feet to " +path[i+1];
          let botVideo = `<video id="botText" width="100%" height=300px autoplay muted> <source src="../maps/v`+userText+`.mp4" type="video/mp4" /></video>`
          $("#mapRedirect").append(botVideo);
        }
    }
    else{
